...
{
    backbone = "./data/nanotrack_backbone.onnx";
    neckhead = "./data/nanotrack_head.onnx";
...
public:
    ...
    float getTrackingScore() override;
    ...
    struct trackerConfig
    {
        ...
        // as in opencv
        //float windowInfluence = 0.490f;
        //float penaltyK = 0.150f;
        //float lr = 0.385f;

        // as in ncnn_macos_nanotrack
        float windowInfluence = 0.462f;
        float penaltyK = 0.148f;
        float lr = 0.390f;
...
{
    const int sz = scoreSize;
    const int stride = trackState.totalStride;
    const int ori = - (sz / 2)*stride;

    std::vector<float> x1Vec(sz, 0);

    for (int i = 0; i < sz; i++)
    {
        x1Vec[i] = (float)(ori + i*stride);
    }
...
void TrackerNanoImpl::init(cv::InputArray image_, const cv::Rect &boundingBox_)
{
    ...
    //scoreSize = trackState.outputSize;
...
void TrackerNanoImpl::getSubwindow(cv::Mat& dstCrop, cv::Mat& srcImg, int originalSz, int resizeSz)
{
    ...
    cv::resize(cropImg, dstCrop, cv::Size(resizeSz, resizeSz));
}
...
bool TrackerNanoImpl::update(cv::InputArray image_, cv::Rect &boundingBoxRes)
{
    ...
    std::vector<cv::String> lns = neckhead.getLayerNames();
    for(cv::String s: lns) {
        printf("Layer: %s\n", s.c_str());
    }
    ...
    for(int i=0; i<clsScore.dims; i++) {
        printf("dim %d\n size: %d", i, clsScore.size[i]);
    }
    ...
    cv::Mat sc = sizeCal(predX2 - predX1, predY2 - predY1)/sizeCal(targetSz[0], targetSz[1]);
    ...
    //float diffXs = (predXs) / scale_z;
    //float diffYs = (predYs) / scale_z;
...
cv::Ptr<TrackerNano> TrackerNano::create(const TrackerNano::Params& parameters)
{
